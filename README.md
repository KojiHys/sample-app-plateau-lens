...
初期視点は、神田・丸の内周辺を南側の上空から北向きに俯角60度（真上から30度傾けた角度）、距離1,000 mで見下ろす位置です（`web/src/cesium/viewer.ts`の`INITIAL_OBLIQUE_PITCH`と`INITIAL_OBLIQUE_RANGE_METERS`）。3Dでは`Ctrl`＋ドラッグ、ホイール押し込み＋ドラッグ、2本指の同方向ドラッグで傾きを変えられます。
